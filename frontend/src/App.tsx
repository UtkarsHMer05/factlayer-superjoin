...
export default function App(){
  ...
  const readOnly=health.mode==='sample'||health.read_only===true;
  ...
  return <div className="app">
    <aside className="sidebar">
      ...
      <div className="collection-control"><label htmlFor="collection">Knowledge collection</label><select id="collection" value={cid} onChange={e=>{setCid(e.target.value);setItems([]);setTotal(0);setLoading(true);setDetail(null);setSource(null);setOffset(0)}}><option value="" disabled>Select a collection</option>{collections.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><button className="quiet add-collection" disabled={readOnly} onClick={()=>setAdding(!adding)}><Plus size={15}/>New collection</button>
    ...
    <main>
      ...
      <div className="main-content">
        ...
        {!cid?<div className="empty"><Layers size={36}/><h2>Start with your evidence.</h2><p>Create a collection, then upload PDFs to discover facts and compare their sources.</p><button className="primary" disabled={readOnly} onClick={()=>setAdding(true)}><Plus size={16}/>Create a collection</button></div>:<>
