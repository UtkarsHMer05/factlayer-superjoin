...
The model adapter sends GLM-5.3's supported top-level `thinking: {"type":"enabled"}`, configurable low reasoning effort, `response_format: {"type":"json_object"}`, and a configurable `max_tokens` limit. It uses a configurable streaming-idle timeout (45 seconds by default), stops on read timeouts instead of retrying them three times, and retries other transient/invalid-JSON failures at most three times. Recent bounded requests reached TokenRouter; an empty final response is now a single resumable error instead of three quota-spending JSON retries. API keys are kept only in Authorization headers.
...
- 52 tests pass, covering comparison arithmetic/negation/context, provenance, rotated PDFs, upload/dedup, admission limits, lease recovery, unavailable model, saved restoration, reprocess, request budgets, request shape, explicit qualifier retention, ranges, modality/polarity, suppression of noisy same-document dated-series pairs, safe startup failure when a port is already occupied, and an empty model response that must not trigger retries. Ruff and frontend build are included in `make test`.
...
1. Run `make test`, `uv run python -m scripts.status`, and inspect git status. Keep `.env` private. The bounded availability check now reaches the configured model; validate one actual source region with `model.extract` before any full pilot. Record status, elapsed time, usage and failure type without logging credentials or hidden model reasoning.
